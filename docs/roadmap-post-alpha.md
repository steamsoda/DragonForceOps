...
- Current preview candidate: `v1.17.12` is aligned with production after fixing the default squad organizer's PostgreSQL conflict-target ambiguity and adding clearer failure messages without changing roster, registration, finance, attendance, or training-group data
...
| 🟢 | Program/Nivel deprecation and tournament-squad transition | Production through `v1.17.12` contains legacy Nivel, adds invited-group roster views/exports, establishes independent competition squads, ships the safe default organizer, and fixes its first-write conflict target. Azul/Blanco remains next. | Nueva Inscripcion, Jugadores, Inscripciones Torneos, `docs/planning/training-groups-model-analysis.md` |
