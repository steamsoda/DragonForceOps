...
| 🟢 | Caja account credit ledger / partial refund model | Planning spec in `docs/planning/caja-account-credit-ledger-plan.md`; `v1.16.154`-`v1.16.157` add the explicit ledger, Caja application, and reassignment remainders. Preview `v1.16.226` adds transactional paid-charge annulment into explicit credit plus a clearer one-confirmation Caja application path. Future passes: legacy implicit-credit review/conversion and deeper credit reporting. Legacy implicit credits stay warning-only until manually reviewed. |
