...
| 33 | **Stripe payment recording + reconciliation** | ✅ Done | Manual reconciliation v1 is live at `/reports/external-payments`: external payments are staged in `external_payment_events`, staff can register/match/ignore them, suggested player + tuition month matching comes from 360Player invoice data, and matched rows create normal internal `stripe_360player` payments/folios without entering cash sessions. Phase B later: import/webhook automation. |
...
| 57 | **Corte Diario + cash session revamp** | 🔴 Open | High-priority finance polish. Clarify the difference between Corte Diario and cash sessions, reduce button overload, show product detail lines in ledger/report views, and add a printable full Corte Diario report distinct from receipt printing. |
