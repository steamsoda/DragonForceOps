...
Last reorganized: 2026-05-06. Last checkpoint: 2026-08-10 (`v1.17.13`).
...
- Current preview candidate: `v1.17.13` adds the guarded one-training-group Azul/Blanco tournament split editor without changing registration, finance, attendance, enrollment, or training-group truth
...
| 🟡 | Program/Nivel deprecation and tournament-squad transition | Production through `v1.17.12` contains legacy Nivel and the safe default organizer. Preview `v1.17.13` adds guarded one-group Azul/Blanco creation and re-editing. Combined groups, helper players/exclusions, snapshots, and handoff remain open. | Nueva Inscripcion, Jugadores, Inscripciones Torneos, `docs/planning/training-groups-model-analysis.md` |
...
| Status | Item | Notes |
|---|---|---|
| 🟡 | `Inscripciones Torneos` tournament-squad workflow | Production `v1.17.10` establishes the independent model, `v1.17.11` ships the default organizer, and `v1.17.12` fixes its first-write conflict. Preview `v1.17.13` adds the guarded one-group Azul/Blanco editor. Next: combined source groups, manual helpers/exclusions, snapshot capture, and export/Convocatorias handoff. |
