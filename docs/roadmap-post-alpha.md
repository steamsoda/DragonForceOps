...
Current preview release line: `v1.16.118`
...
2. Historical regularization workspace hardening
   ...
   - remaining follow-up:
     ...
     - `v1.16.114` removes remaining eager workspace server actions and narrows the Mensualidad context query so it no longer loads the full account ledger just to list tuition periods
...
4. Collections / pending-tuition board split
   ...
   - follow-up:
     ...
     - `Llamadas` workflow polish:
       - `v1.16.118` starts pass 1 by preserving the existing follow-up notes/status model, adding a follow-up status filter, routing `No regresara` to the dedicated baja workflow, and adding inline audited baja capture from the call row
       - baja from `Llamadas` ends the active enrollment only; charge writeoff remains director-only in `Bajas y saldos pendientes`
       - pass 2 should rebuild `Llamadas` navigation around the proven `Pendientes` campus/category/bucket cards and align the queue source more explicitly with pending monthly tuition
...
6. Attendance tracking v1
   ...
   - continuation follow-up:
     ...
     - `v1.16.116` changes the calendar day interaction from inline expansion to a day-detail redirect into `/attendance?date=YYYY-MM-DD`, preserving campus selection and avoiding stretched calendar cells
...
10. Nutrition v2 metrics and parent report exports
   - `v1.16.78` starts this lane:
     ...
     - adds a printable parent report page with OMS chart section, measurement summary/history, waist trend, and notes box
   - `v1.16.79` polishes the parent report into a dedicated single-page-oriented layout:
     - top narrative notes/recommendations area
     - right-side measurement sidebar
     - compact OMS charts and recent history
   - `v1.16.80` refines parent-facing copy and print layout:
   ...
   - `v1.16.81` tightens live nutrition capture:
     ...
     - OMS charts render all captures in the same age-month
   - `v1.16.82` refines the print/PDF path:
     - interactive OMS charts return to full reference-range axes
     - parent report percentile graphic becomes a bell-curve style marker
     - compact report OMS charts use print-safe static SVG instead of responsive Recharts
     - empty report notes no longer print placeholder text
   - `v1.16.83` fixes the main profile OMS chart axis after the repeated-measurement series change:
     - profile charts now explicitly use the WHO/OMS reference age range
     - repeated same-age player points remain supported without collapsing the reference curve
