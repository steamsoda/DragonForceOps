...
- Current preview candidate: `v1.16.234` (promoted)
- Training workload status: passes 1-4 are production-ready in `v1.16.234`: coach snapshots, rolling RPC, `Por coach`, `Por horario`, role/data-boundary regression, measured performance, and snapshot-quality monitoring. Print-preview refinement is the next polish pass.
...
|---|---|---|---|
| ✅ | Training workload by coach and time block | Production `v1.16.231`-`v1.16.234` includes coach snapshots, one rolling RPC, compact coach/schedule matrices, separate `+P` tryouts, three averages, role regression, measured query performance, and explicit snapshot-quality monitoring. Print-preview refinement remains a separate polish task. | `docs/planning/training-workload-report-plan.md`, Reports / Asistencia |
