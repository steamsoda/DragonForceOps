...
- Current preview line: `v1.16.218`
...
| 🟢 | Enrollment second tutor + product-charge override | Preview `v1.16.218` adds optional second-tutor capture during intake and a tightly guarded Director Admin/Super Admin override for one untouched product charge. Validate intake rollback, Caja handoff, audit history, and finance sanity before production. | New Enrollments / Caja, `v1.16.218` devlog |
...
| 🟢 | Tryout classes workflow | Preview `v1.16.210`-`v1.16.217` implements Passes 1-4 plus visitor reporting detail, date ranges, and YOB distribution. Pass 5 adds explicit abandoned/declined closure, fourth-class override, and duplicate hardening without deleting trial history. | `docs/planning/trial-classes-plan.md`, `v1.16.210`-`v1.16.217` devlog |
