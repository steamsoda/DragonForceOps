...
- Current preview line: `v1.16.153`
...
| 🟡 | Caja account credit ledger / partial refund model | Planning spec added in `docs/planning/caja-account-credit-ledger-plan.md`. This is the deliberate finance-foundation path for explicit credit entries, legacy-credit review, receipt/report visibility, and drift-safe mixed-payment handling. |
...
| 🟡 | Caja account credit ledger / partial refund model | Planning spec added in `docs/planning/caja-account-credit-ledger-plan.md`. Future finance-foundation pass for mixed payments where a non-refundable/protected part stays put but an overage or eligible source amount exceeds the destination charge, such as tournament money becoming next-month tuition credit. Explicit credits must be additive; legacy implicit credits stay warning-only until reviewed and manually converted. |
