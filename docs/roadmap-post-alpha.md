...
Last reorganized: 2026-05-06. Last checkpoint: 2026-08-09 (`v1.17.12`).
...
- Current production line: `v1.17.11`
- Current preview candidate: `v1.17.12` fixes the default squad organizer's PostgreSQL conflict-target ambiguity and adds clearer failure messages without changing roster, registration, finance, attendance, or training-group data
...
| 🟢 | Program/Nivel deprecation and tournament-squad transition | Production through `v1.17.11` contains legacy Nivel, adds invited-group roster views/exports, establishes independent competition squads, and ships the safe default organizer. Preview `v1.17.12` fixes its first-write conflict target. Azul/Blanco remains next. | Nueva Inscripcion, Jugadores, Inscripciones Torneos, `docs/planning/training-groups-model-analysis.md` |
...
| Status | Item | Notes |
|---|---|---|
| 🟡 | `Inscripciones Torneos` tournament-squad workflow | Production `v1.17.10` establishes the independent model and `v1.17.11` ships the transactional default squad organizer; preview `v1.17.12` fixes its first-write conflict. Next: Azul/Blanco splits, combined source groups, manual helpers/exclusions, snapshot capture, and export/Convocatorias handoff. |
