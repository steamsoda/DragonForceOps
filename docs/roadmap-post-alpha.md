...
Current preview release line: `v1.12.0`
...
| 56 | **Refund workflow** | 🟡 In progress | `v1` now adds two explicit payment-side workflows instead of relying on the confusing "void charge -> leftover credit" workaround: `Cambiar concepto` can move a full posted payment onto new destination charges and auto-void its exclusive original source charges, while `Reembolsar` records a separate refund movement on the refund date, reopens the underlying balance, and surfaces refund state in receipts/activity/reporting. Keep open for future partial refunds and any deeper finance-op guardrails after live usage. |
