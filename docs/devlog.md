...
- Verified Main and Preview at cfa091d before release. Preview had exactly one
  pending migration, 20260923010000. Saved schema-only baseline and installed it
  transactionally before the dependent application. Existing coaches unchanged.
- Added Preview-pinned synthetic hosted tests for save/refresh persistence,
  read-only denial and existing-session/provider departure revocation, with
  tracked cleanup. Hosted results pending deployment; production untouched.

## 2026-09-22: Profesores local implementation (INV-PLAN-002A r1)

- User accepted the reviewed handoff. Built the campus-grouped coach directory,
  side-panel create/edit/assignment/replacement controls and guarded departure.
  Calendar editing and support tickets remain out of scope.
- Production inventory was read-only: 14 active coaches, 8 linked accounts, 50
  assignments; checked assignment/schedule anomalies absent. One linked account
  has another role, confirming account-wide departure protection is necessary.
- Added a pending migration for atomic stale-safe assignments, mandatory audit,
  historical attendance attribution locks, persistent account denial, protected
  account safeguards, and no automatic reactivation/regrant. Provider banning
  follows the committed local denial with an explicit retry state.
- Local checks: 57 rollback-only coach DB assertions, 111 existing finance/receipt
  DB assertions with the new guards, 11 mocked action assertions, 19 browser UI
  assertions, 7 combined-team summary checks, shared proxy regression, and full
  webpack/TypeScript build passed.
- User approved inherited tournament teams following training-group coach changes.
  Review now includes before/after team responsibility and unstaffed warnings;
  stale tournament reviews reject atomically. Manual assignments and the existing
  organizer combined-team manual default remain unchanged. Historical attendance
  remains intact. This amends the r1 inheritance restriction without editing the
  handed-off planning document. No real tournament assignments changed.
- No commit/push, installed migration, deployed app, real coach/account mutation,
  or provider change. Hosted session/provider verification remains a Preview gate.
  Review and release requirements: `docs/planning/profesores-implementation-review.md`.

## 2026-09-22: v1.17.79 deployed to production

- Owner approved promotion after Preview acceptance. Main and Preview both point
  to `cfa091da3b5b5237949fa682811d8b979b5a51ac`; no uncommitted application changes
  were included. The release includes explicit-credit checkout and operation receipts.
- Production preflight found exactly the eight reviewed `20260922010000` through
  `20260922080000` migrations pending. Schema-only baseline saved locally. All eight
  applied successfully to `hjvytfaalnfcqfgbxsmj`; postflight confirmed no backlog,
  six private RLS tables, disabled automatic credit and blocked receipt mutations
  and core-routine bypass. No production financial rehearsal or repair was run.
- Clean-cache production build passed with TypeScript enabled. Production alias
  verified at `dpl_HZZNYyGuLUpXsYt3LyNT4KyZ2g8D` / `dragon-force-r5maqnwk7-steamsodas-projects.vercel.app`.
  The superseded cached build was cancelled. Migration, auth contract, dependency
  security audit and secret scan workflows all passed.
- 46 authenticated production smoke checks passed for Director, Front Desk and
  Director Read Only, including live version/routes, receipt history access,
  anonymous/cross-campus denial, no-store responses and blocked read-only writes.
  The synthetic login and roles were removed. No payments, refunds or emails were
  generated by validation. Caja pause was released after successful checks.
- Physical printing was not exercised by automation; existing QZ transport remains
  unchanged, and cancellation/refund receipts use saved operation snapshots.

## 2026-09-22: v1.17.79 operation receipts deployed to Preview

- Preview commit: `cfa091da3b5b5237949fa682811d8b979b5a51ac`.
  Ready deployment: `dpl_8VVSriZqVzBEK4NECdr1cHk8958Y`; stable Preview alias
  explicitly assigned to `dragon-force-ot01936l8-steamsodas-projects.vercel.app`.
- Receipt migration `20260922080000` installed. Verified RLS, no authenticated
  direct table reads, no service-role receipt updates or bypass of the wrappers.
  Installed-schema rollback-only regression passed 111 checks.
- Clean-cache cloud build, TypeScript, migration workflow, auth contract,
  dependency audit and secret scan passed. Cancelled the superseded cached build.
- 46 hosted checks passed across Director, Front Desk and Director Read Only:
  correct app version/routes, saved-history-only receipt lookup, anonymous and
  cross-campus denial, no-store responses and read-only mutation/print denial.
  Temporary identity and roles removed; no financial writes or emails in smoke.
- Production main remains `9244cde3b683110f55936d5496dfc479163b2a0a` and production
  deployment remains `dpl_HFMhJmERU8bbyHS4J7oFFhj5BawC`. Physical receipt printing
  and user acceptance remain pending; no production release authorized.
