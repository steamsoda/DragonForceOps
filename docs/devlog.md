...
- Promoted the verified hotfix through Preview to production as `v1.17.63`; the production Vercel deployment completed successfully and no database migration was required.
...
- Added clearer organizer error classification for permission, scope, advanced-squad, and database-version failures while keeping internal database details in server logs.
- Added a focused regression assertion for the unambiguous conflict target and verified that the fix migration contains no writes to finance, registrations, or training assignments.
...
- Added a clear active state, filtered-player count, `Mostrar todos` action, empty-result message, and preserved the filter when changing the month.
- Kept the KPI and filtered roster aligned through one explicit backend-derived per-player flag; no attendance records, group assignments, permissions, finance data, or schema behavior changed.
- Verification: `npm run typecheck` and `npm run build`.
