...
(async () => {
  ...
  check(result.receipt.birthYear === null, 'Legacy snapshot does not invent a category from current profile');
  saved.birthYear = 2015;
  payment.enrollments.players.birth_date = '2014-01-01';
  check((await get()).receipt.birthYear === 2015, 'Reprint uses saved birth year even after profile changes');
