...
export async function acknowledgeExplicitCajaCartAction() { await fetch('/test-recovery', { method: 'DELETE' }); }
...
export async function checkoutCajaCartAction(_: string, form: FormData) {
  ...
  return { ok: true as const, receipt: { operationId: command.requestId, enrollmentId: id, actorId: id,
