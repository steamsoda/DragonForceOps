// Production parent-go gate: do not run setup/recovery/revoke/cleanup until explicitly requested.
// Synthetic production identity only; no emails, migration apply, or academy mutations.
// Suggested sequence: setup, http, revoke, recovery, cleanup. Recovery invalidates saved sessions.
...
const PURPOSE = "temporary_director_readonly_production_validation_v1";
const HOSTED = "https://dragon-force-ops.vercel.app";
const ROOT = path.resolve(__dirname, "..");
const DIR = path.join(ROOT, ".tmp");
const STATE = path.join(DIR, "director-readonly-production-validation.json");
const BROWSER = path.join(DIR, "director-readonly-production-browser-state.json");
...
function targetOrigin(value) {
  ...
  check(url.origin === HOSTED, "production_target_not_allowlisted");
  return url.origin;
}
function args(argv) {
...
function load() {
  ...
  check(state.email === `director-readonly-production-validation-${state.runId}@example.invalid`, "invalid_fixture_email");
...
async function main(argv) {
  ...
  safeFiles();
  const env = parseEnv(fs.readFileSync(path.join(ROOT, ".env.prod.local"), "utf8"));
  const api = new URL(env.NEXT_PUBLIC_SUPABASE_URL);
  const dbUrl = new URL(env.SUPABASE_PROD_DB_URL);
  check(api.origin === `https://${REF}.supabase.co`, "production_api_required");
  check(dbUrl.hostname === `db.${REF}.supabase.co` || (dbUrl.hostname.endsWith(".pooler.supabase.com") && decodeURIComponent(dbUrl.username) === `postgres.${REF}`), "production_db_required");
  check(dbUrl.pathname === "/postgres", "production_database_required");
  dbUrl.searchParams.delete("sslmode");
  const publicKey = env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY || env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
  check(publicKey && env.SUPABASE_SERVICE_ROLE_KEY, "production_keys_required");
  ...
  async function recoveryProof(state) {
    ...
    check(typeof state.refreshToken === "string", "saved_refresh_token_required");
    const client = jwtClient(state);
    const identity = await client.auth.getUser(state.accessToken);
    check(!identity.error && identity.data.user?.id === state.userId, "recovery_initial_session_invalid");
    // pgcrypto computes a boolean inside DB; neither password hashes nor passwords are logged.
    ...
    const newPassword = "Dr!" + randomUUID();
    ...
    const sessionsBefore = await query("select count(*)::int as count from auth.sessions where user_id=$1", [state.userId]);
    check(sessionsBefore[0].count > 0, "recovery_has_no_existing_sessions");
    const link = await admin.auth.admin.generateLink({ type: "recovery", email: state.email });
    check(!link.error && link.data.user?.id === state.userId && link.data.properties?.hashed_token, "synthetic_recovery_link_failed");
    state.recoveryAttemptedAt = new Date().toISOString(); save(state);
    // No forgot/resend/signin request: no messages or fake CAPTCHA tokens.
    const response = await request(state, options.target, "/api/auth/password", {
      method: "POST", headers: { Origin: HOSTED, "Content-Type": "application/json" },
      body: JSON.stringify({ action: "reset", token_hash: link.data.properties.hashed_token, password: newPassword }),
    });
    check(response.status === 200, `recovery_backend_failed status=${response.status}`);
    check(await passwordMatches(), "recovery_password_not_changed");
    check(!response.headers.getSetCookie().some((cookie) => cookie.startsWith(`sb-${REF}-auth-token`) && !/Max-Age=0/i.test(cookie)), "recovery_returned_live_auth_cookie");
    ...
    console.log("PASS synthetic recovery: app reset backend changed password; all fixture sessions removed and old refresh rejected. No email/CAPTCHA claim; unexpired access JWTs may remain valid until expiry. Run guarded cleanup.");
  }
  ...
  try {
    if (options.mode === "--setup") {
      ...
      const runId = randomUUID(), email = `director-readonly-production-validation-${runId}@example.invalid`;
      ...
      if (options.mode === "--browser-state") {
        ...
        console.log("Browser storageState ready: .tmp/director-readonly-production-browser-state.json (secrets not printed; load only on requested allowlisted origin).");
      }
      ...
      if (options.mode === "--http") {
        ...
        check(checked === 22 && testedDetails.size >= 20 && Object.values(detailCoverage).every((n) => typeof n === "number" && n > 0), `production_coverage_incomplete pages=${checked} details=${testedDetails.size} coverage=${JSON.stringify(detailCoverage)}`);
