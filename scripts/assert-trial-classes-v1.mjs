...
assert(action.includes("confirmPossibleDuplicate") && action.includes("trial_prospect.created_duplicate_override"), "Possible duplicate overrides must be explicitly confirmed and audited");
...
assert(prospectForm.includes("Este jugador posiblemente ya esta registrado") && prospectForm.includes("Si, registrar"), "Possible duplicates need an explicit operator confirmation");
assert(prospectForm.includes('formData.set("confirmPossibleDuplicate", "true")'), "Duplicate confirmation must be sent explicitly to the server");
