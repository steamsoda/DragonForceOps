...
import { RESTRICTED_VALUE, type DashboardPresentation } from "@/lib/queries/dashboard-presentation";
...
function formatCurrency(value: number | null) {
  if (value === null) return RESTRICTED_VALUE;
...
export function DashboardView({ campuses, dashboard, selectedCampusId }: {
  campuses: { id: string; name: string }[];
  dashboard: DashboardPresentation;
  selectedCampusId: string;
}) {
  const upToDate = dashboard.enrollmentsWithBalance === null ? null : dashboard.activeEnrollments - dashboard.enrollmentsWithBalance;
  return (
    <PageShell title="Panel" subtitle="Resumen operativo de Fase 1">
      <div className="space-y-4">
        ...
        <div className="grid gap-3 md:grid-cols-4">
          <KpiCard label="Alumnos con saldo" value={dashboard.enrollmentsWithBalance === null ? RESTRICTED_VALUE : dashboard.enrollmentsWithBalance.toLocaleString("es-MX")} description="Inscripciones activas con adeudo pendiente" />
          <KpiCard
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <div className="rounded-md border border-amber-200 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-950/30">
            ...
            <p className="mt-1 text-xs text-slate-600 dark:text-slate-400">
              {dashboard.player360Count ?? RESTRICTED_VALUE} pago{dashboard.player360Count !== 1 ? "s" : ""} incluidos dentro del mes seleccionado
            </p>
          </div>
          <div className="rounded-md border border-sky-200 bg-sky-50 p-4 dark:border-sky-800 dark:bg-sky-950/30">
            ...
            <p className="mt-1 text-xs text-slate-600 dark:text-slate-400">
              {dashboard.historicalCatchupCount ?? RESTRICTED_VALUE} pago{dashboard.historicalCatchupCount !== 1 ? "s" : ""} contados por su fecha real de pago
            </p>
          </div>
        </div>
