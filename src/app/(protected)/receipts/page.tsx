      ...
      <div className="space-y-4">
        ...
        <form method="GET" className="flex flex-wrap gap-3">
          <input
            ...
            className="flex-1 min-w-[220px] rounded-md border border-slate-300 dark:border-slate-600 px-3 py-2 text-sm bg-white dark:bg-slate-800"
          />
          <select
            name="campus"
            defaultValue={campusId}
            className="rounded-md border border-slate-300 dark:border-slate-600 px-3 py-2 text-sm bg-white dark:bg-slate-800"
          >
              ...
              {result.rows.length === 0 ? (
                ...
                result.rows.map((row) => (
                  <tr key={row.paymentId}>
                    ...
                    <td className="px-3 py-2 whitespace-nowrap">{formatDate(row.paidAt)}</td>
        ...
        {(q || campusId) && result.total > result.pageSize && (
          <div className="flex items-center justify-between text-sm">
            ...
            <div className="flex gap-3">
              {prevPage ? (
                <Link href={`/receipts?${qsBase}&page=${prevPage}`} className="rounded border px-3 py-1.5 hover:bg-slate-50 dark:hover:bg-slate-800">Anterior</Link>
              ) : (
                <span className="rounded border px-3 py-1.5 text-slate-400">Anterior</span>
              )}
              {nextPage ? (
                <Link href={`/receipts?${qsBase}&page=${nextPage}`} className="rounded border px-3 py-1.5 hover:bg-slate-50 dark:hover:bg-slate-800">Siguiente</Link>
              ) : (
