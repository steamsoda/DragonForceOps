      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <div className="rounded-md border border-slate-200 bg-slate-50 p-4">
            ...
            <p className="text-xs text-slate-500 mt-1">{data.payments.length} pago{data.payments.length !== 1 ? "s" : ""}</p>
          </div>
          {data.byMethod.map((m) => (
            <div key={m.method} className="rounded-md border border-slate-200 bg-slate-50 p-4">
              <p className="text-xs text-slate-500 uppercase tracking-wide">{m.methodLabel}</p>
              <p className="mt-1 text-2xl font-semibold text-slate-900">{fmt(m.total)}</p>
              <p className="text-xs text-slate-500 mt-1">{m.count} pago{m.count !== 1 ? "s" : ""}</p>
            </div>
          ))}
        </div>
              ...
              <tbody className="divide-y divide-slate-100">
                {data.payments.map((p) => (
                  <tr key={p.id} className="hover:bg-slate-50">
                    <td className="px-3 py-2 text-slate-500">{fmtTime(p.paidAt)}</td>
                    <td className="px-3 py-2">
                      <Link href={`/enrollments/${p.enrollmentId}/charges`} className="text-portoBlue hover:underline">
                        {p.playerName}
                      </Link>
                    </td>
                    <td className="px-3 py-2 text-slate-600">{p.methodLabel}</td>
