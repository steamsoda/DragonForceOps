...
export default async function HistoricalRegularizationPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const successMessage =
    params.ok === "historical_payment_posted"
      ? "Pago histórico registrado correctamente."
      : params.ok === "payment_reassigned"
        ...
        : params.ok === "payment_refunded"
          ...
          : null;
  const errorMessage = params.err ? errorMessages[params.err] ?? "Ocurrió un error en la regularización histórica." : null;

  return (
    <PageShell
      title="Regularización histórica"
      subtitle="Workspace excepcional para captura atrasada, creación o recálculo de mensualidades y correcciones históricas sin usar Caja."
      breadcrumbs={[{ label: "Super Admin", href: "/admin/users" }, { label: "Regularización histórica" }]}
      wide
    >
      <div className="space-y-5">
        <div className="rounded-md border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-200">
          <p className="font-semibold">Superadmin únicamente</p>
          <p>
            Esta pantalla registra movimientos históricos con fecha real. No forma parte del flujo normal de Caja, no imprime
            automáticamente y no enlaza pagos a una sesión de caja abierta.
          </p>
          ...
          <section className="space-y-4">
            {!selectedLedger ? (
              <div className="rounded-md border border-dashed border-slate-300 px-4 py-8 text-sm text-slate-500 dark:border-slate-700 dark:text-slate-400">
                Selecciona un campus y luego una cuenta para revisar cargos, aplicar pagos históricos o crear mensualidades con
                fecha efectiva real.
              </div>
            ) : (
              <ContryRegularizationAccountPanel initialLedger={selectedLedger} />
            )}
          </section>
        </div>
      </div>
