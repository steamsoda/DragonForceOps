...
import type { CompetitionRosterSnapshotExportData } from "@/lib/queries/competition-rosters";
...
export async function buildCompetitionRosterSnapshotWorkbook(data: CompetitionRosterSnapshotExportData) {
  const workbook = new ExcelJS.Workbook();
  workbook.creator = "Dragon Force Monterrey";
  workbook.created = new Date();

  const worksheet = workbook.addWorksheet("Plantel aprobado");
  ...
  const title = worksheet.addRow([`${data.tournamentName} - Plantel aprobado`]);
  ...
  const subtitle = worksheet.addRow([
    `${data.campusName} | ${data.label} | ${data.squads.reduce((total, squad) => total + squad.members.length, 0)} lugares`,
  ]);
  ...
  for (const squad of data.squads) {
    worksheet.addRow([]);
    const squadTitle = worksheet.addRow([
      `${squad.name} (${squad.members.length} jugadores)`,
    ]);
    ...
    squad.members.forEach((member, index) => {
      const row = worksheet.addRow([
        ...
        member.source === "manual" ? "Refuerzo" : "Inscripcion pagada",
      ]);
