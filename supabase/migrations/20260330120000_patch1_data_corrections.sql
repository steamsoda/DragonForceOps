...
BEGIN
  IF EXISTS (
    ...
    JOIN public.payment_allocations pa ON pa.charge_id = c.id
    WHERE e.player_id = v_del
  ) THEN
    RAISE EXCEPTION 'Patch1: DF-0170 has existing payments — manual intervention required before deletion';
  END IF;

  -- End any active enrollments
  UPDATE public.enrollments
  SET status = 'ended', end_date = '2026-03-30', dropout_reason = 'otro', updated_at = now()
  WHERE player_id = v_del AND status = 'active';

  DELETE FROM public.enrollments WHERE player_id = v_del;
  DELETE FROM public.players WHERE id = v_del;
  RAISE NOTICE 'Patch1: DF-0170 deleted successfully';
END $$;
...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Patch1: DF-0574 has existing payments — manual intervention required before deletion';
  END IF;
  ...
  DELETE FROM public.enrollments WHERE player_id = v_del;
  DELETE FROM public.players WHERE id = v_del;
  RAISE NOTICE 'Patch1: DF-0574 deleted successfully';
END $$;

-- DF-0527 (SEBASTIAN GUZMAN RDZ born 2015): keep DF-0476 (born 2014, real record), delete DF-0527
-- DF-0476 last_name already corrected to 'Guzmán Ramírez' in Section 1
DO $$
DECLARE
...
BEGIN
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Patch1: DF-0527 has existing payments — manual intervention required before deletion';
  END IF;
  ...
  DELETE FROM public.enrollments WHERE player_id = v_del;
  DELETE FROM public.players WHERE id = v_del;
  RAISE NOTICE 'Patch1: DF-0527 deleted successfully';
END $$;


-- ============================================================
-- SECTION 3: BAJAS (4 enrollment ends, dropout_reason = 'otro')
-- ============================================================

-- DF-0668 (LUIS CARLOS LOPEZ HILARIO, LV 2021)
UPDATE public.enrollments
SET status = 'ended', dropout_reason = 'otro', end_date = '2026-03-30', updated_at = now()
