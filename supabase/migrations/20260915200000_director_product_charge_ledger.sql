create or replace function public.director_product_charge_ledger(
  p_product_id uuid,
...
returns table (
  ...
  training_group_program text,
  training_group_birth_year_min integer,
  training_group_birth_year_max integer,
...
as $$
  with target_charges as (
    ...
    where public.is_director_readonly() and auth.uid() is not null
      and charge.enrollment_id in (select id from public.enrollments where campus_id in (select id from public.campuses where is_active))
      and charge.product_id = p_product_id
  ...
  filtered as (
    select
      ...
      active_group.program as training_group_program,
      active_group.birth_year_min as training_group_birth_year_min,
      active_group.birth_year_max as training_group_birth_year_max,
      coalesce(product_teams.assigned_teams, '[]'::jsonb) as assigned_teams,
      charge.description,
      charge.amount,
      charge.currency,
      case
    ...
    left join lateral (
      select jsonb_agg(
        jsonb_build_object(
          ...
          'category_label', squad.category_label,
          'campus_name', squad_campus.name,
        ...
        order by tournament.name, squad.sort_order, squad.name
      ) as assigned_teams
      ...
      join public.campuses squad_campus on squad_campus.id = tournament.campus_id
      where member.enrollment_id = enrollment.id
        and tournament.product_id = charge.product_id
    ) product_teams on true
  ...
  select
    ...
    filtered.training_group_program,
    filtered.training_group_birth_year_min,
    filtered.training_group_birth_year_max,
    filtered.assigned_teams,
...
revoke all on function public.director_product_charge_ledger(uuid, timestamptz, timestamptz, integer, integer) from public, anon;
grant execute on function public.director_product_charge_ledger(uuid, timestamptz, timestamptz, integer, integer) to authenticated;

comment on function public.director_product_charge_ledger(uuid, timestamptz, timestamptz, integer, integer) is
  'Read-only product charge ledger with canonical current-group metadata, matching tournament-team assignments, and exact payment completion timestamps.';
