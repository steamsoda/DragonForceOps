...
as $$
  ...
  eligible_entries as (
    ...
    join allocation_totals paid
     ...
     and paid.allocated_amount + 0.009 >= charge.amount
...
comment on function public.sync_paid_tournament_entries_for_charges(uuid, uuid[]) is
  'Registers newly funded tournament charges in one targeted statement. The tournament entry trigger queues sporting squad routing for the background worker.';
