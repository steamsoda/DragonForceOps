-- Allow eligible Caja payment reassignment remainders to become explicit account credit.
--
-- This only affects the selected-source-charge workflow used by Caja's
-- "Cambiar concepto" screen. Monthly tuition and inscription source charges
-- remain protected by the existing guards.
...
declare
  ...
  v_target_ids uuid[];
  v_target_capacity numeric(12,2) := 0;
  v_amount_to_allocate numeric(12,2) := 0;
...
begin
  ...
  if coalesce(array_length(v_target_ids, 1), 0) > 0 then
    ...
    from (
      select
        c.id,
        greatest(c.amount - coalesce(pa.amount, 0) - coalesce(eca.amount, 0), 0)::numeric(12,2) as pending_amount
      from public.charges c
      left join (
        select charge_id, coalesce(sum(amount), 0)::numeric(12,2) as amount
        from public.payment_allocations
        where charge_id = any(v_target_ids)
        group by charge_id
      ) pa on pa.charge_id = c.id
      left join (
        select charge_id, coalesce(sum(amount), 0)::numeric(12,2) as amount
        from public.enrollment_credit_applications
        where charge_id = any(v_target_ids)
        group by charge_id
      ) eca on eca.charge_id = c.id
      where c.id = any(v_target_ids)
    ) targets;
  end if;

  v_amount_to_allocate := round(least(v_source_amount, v_target_capacity), 2);
  v_credit_amount := round(v_source_amount - v_amount_to_allocate, 2);

  delete from public.payment_allocations pa
  where pa.payment_id = p_payment_id
    and pa.charge_id = p_source_charge_id;

  if v_amount_to_allocate > 0.009 then
    ...
    running as (
      select
        ...
        greatest(
          least(
            pending_amount,
            v_amount_to_allocate - coalesce(sum(pending_amount) over (
              order by ord
  ...
  if v_credit_amount > 0.009 then
    ...
    values (
      ...
      (select e.campus_id from public.enrollments e where e.id = v_payment.enrollment_id),
      p_payment_id,
      p_source_charge_id,
      ...
      v_payment.currency,
      'open',
      'Caja: remanente de cambio de concepto',
      case
        when v_amount_to_allocate > 0.009 then 'Parte del pago se aplico a cargos destino; el remanente quedo como credito.'
        else 'El cargo origen se anulo y el monto quedo como credito en cuenta.'
      end,
      auth.uid()
    );
  end if;
