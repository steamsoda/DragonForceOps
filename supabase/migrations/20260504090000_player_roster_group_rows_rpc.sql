-- Flatten the Jugadores grouped roster into one live RPC result so the app
-- does not need to stitch enrollment, group, charge, allocation, and payment
-- rows together through several PostgREST requests.
...
as $$
  with base as (
    ...
    where public.can_read_attendance_campus(p_campus_id)
...
grant execute on function public.get_player_roster_group_rows(uuid, date, date, date, text, int) to authenticated;
