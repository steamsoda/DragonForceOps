-- Treat inscription charges like monthly tuition in Caja payment workflows.
-- They are visible for audit context, but cannot be refunded or reassigned.
...
begin
  ...
  select ct.code into v_source_type_code from public.charge_types ct where ct.id = v_source_charge.charge_type_id;
  ...
  if v_source_type_code = 'inscription' then
    return query select false, 'source_charge_inscription', null::uuid, null::uuid, null::numeric, p_source_charge_id, null::uuid[];
    return;
  end if;
  ...
  return query
  select true, null::text, v_payment.id, v_payment.enrollment_id, v_source_amount, p_source_charge_id, v_target_ids;
end;
$$;
...
begin
  ...
  if p_refund_method not in ('cash'::public.payment_method, 'card'::public.payment_method) then
    return query select false, 'invalid_refund_method', null::uuid, null::uuid, null::uuid, null::numeric, null::uuid;
    return;
  end if;
  ...
  into v_source_ids, v_source_count, v_allocated_total
  from public.payment_allocations pa
  where pa.payment_id = p_payment_id;
  ...
  if abs(v_allocated_total - v_payment.amount) > 0.01 then
    return query select false, 'payment_not_fully_allocated', null::uuid, null::uuid, null::uuid, null::numeric, null::uuid;
    return;
  end if;
  ...
  if exists (
    ...
    return query select false, 'source_charge_monthly_tuition', null::uuid, null::uuid, null::uuid, null::numeric, null::uuid;
    return;
  end if;
  ...
  if exists (
    ...
    join public.charge_types ct on ct.id = c.charge_type_id
    where c.id = any(v_source_ids)
      and ct.code = 'inscription'
  ) then
    return query select false, 'source_charge_inscription', null::uuid, null::uuid, null::uuid, null::numeric, null::uuid;
    return;
  end if;
  ...
  if exists (
    ...
    return query select false, 'source_charge_shared', null::uuid, null::uuid, null::uuid, null::numeric, null::uuid;
    return;
  end if;
  ...
  if exists (
    ...
    from (
      ...
      left join public.payment_allocations pa on pa.charge_id = c.id
      where c.id = any(v_source_ids)
      group by c.id, c.amount, c.status
    ) source_state
    where source_state.status = 'void'
      or abs(source_state.amount - source_state.allocated_amount) > 0.01
  ) then
    return query select false, 'source_charge_not_exclusive', null::uuid, null::uuid, null::uuid, null::numeric, null::uuid;
    return;
  end if;
  ...
  delete from public.payment_allocations pa where pa.payment_id = p_payment_id;
  ...
  return query
  select true, null::text, v_refund_id, v_payment.id, v_payment.enrollment_id, v_payment.amount, v_operator_campus_id;
exception
  when not_null_violation or foreign_key_violation or check_violation then
    return query
    select false, 'refund_insert_failed', null::uuid, v_payment.id, v_payment.enrollment_id, v_payment.amount, coalesce(v_operator_campus_id, v_payment.operator_campus_id);
  when others then
    return query
    select false, 'refund_failed', null::uuid, v_payment.id, v_payment.enrollment_id, v_payment.amount, coalesce(v_operator_campus_id, v_payment.operator_campus_id);
end;
$$;
...
begin
  ...
  if exists (
    ...
    return query select false, 'source_charge_monthly_tuition', null::uuid, null::uuid, null::numeric, v_source_ids, null::uuid[];
    return;
  end if;
  ...
  if exists (
    ...
    return query select false, 'source_charge_inscription', null::uuid, null::uuid, null::numeric, v_source_ids, null::uuid[];
    return;
  end if;
  ...
  return query
  select true, null::text, v_payment.id, v_payment.enrollment_id, v_payment.amount, v_source_ids, v_target_ids;
end;
$$;
