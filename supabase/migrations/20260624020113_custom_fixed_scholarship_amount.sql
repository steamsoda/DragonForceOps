alter table public.enrollments
  add column if not exists custom_scholarship_amount numeric(10,2);

comment on column public.enrollments.custom_scholarship_amount is
  'Fixed monthly tuition amount when scholarship_status = custom. Null for none, half, and full scholarships.';

alter table public.enrollments
  drop constraint if exists enrollments_scholarship_status_check;

alter table public.enrollments
  add constraint enrollments_scholarship_status_check
  check (scholarship_status in ('none', 'half', 'full', 'custom'));

alter table public.enrollments
  drop constraint if exists enrollments_custom_scholarship_amount_check;

alter table public.enrollments
  add constraint enrollments_custom_scholarship_amount_check
  check (
    (
      scholarship_status = 'custom'
      and custom_scholarship_amount is not null
      and custom_scholarship_amount > 0
    )
    or (
      scholarship_status <> 'custom'
