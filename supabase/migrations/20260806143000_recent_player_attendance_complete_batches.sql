-- Keep the roster/export last-N attendance contract aligned with the app.
-- Client-side chunks are sized so p_player_ids * p_limit stays below PostgREST's row cap.
...
as $$
  ...
  from ranked_records
  where ranked_records.row_number <= greatest(1, least(coalesce(p_limit, 5), 15))
