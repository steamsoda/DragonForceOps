-- Individual receipts only. Does not grant raw financial-table access.
create or replace function public.director_search_receipts(
  p_query text default null, p_campus_id uuid default null,
  p_payment_id uuid default null, p_limit int default 30, p_offset int default 0
)
returns table (
...
as $$
  with allowed_campuses as materialized (
    ...
    where c.is_active and (select auth.uid()) is not null
      and (select public.is_director_readonly())
  ), params as (
    select nullif(trim(left(p_query, 150)), '') as q,
    ...
    join allowed_campuses ac on ac.id = e.campus_id
    ...
    where p.status = 'posted'
      ...
      and (prm.q is null or coalesce(p.folio, '') ilike '%' || prm.q || '%'
...
$$;
revoke all on function public.director_search_receipts(text, uuid, uuid, int, int) from public, anon;
grant execute on function public.director_search_receipts(text, uuid, uuid, int, int) to authenticated;
