...
revoke all on function public.reprice_unallocated_charge(uuid, numeric) from public;
revoke all on function public.reprice_unallocated_charge(uuid, numeric) from anon;
revoke all on function public.reprice_unallocated_charge(uuid, numeric) from authenticated;
grant execute on function public.reprice_unallocated_charge(uuid, numeric) to service_role;

comment on function public.reprice_unallocated_charge(uuid, numeric) is
  'Atomically overrides one pending, untouched charge. Service-role only; caller must enforce superadmin authorization and audit the reason.';

drop function if exists public.reprice_unallocated_product_charge(uuid, numeric);
